...
namespace PlanGoGoService
{
    ...
    public class GetListValues : BaseReference,IGetListValues
    {
        ...
        public List<GetOrderOfAttractionVisit> Public_GetOrderOfAttractionVisit(int TravelModeId, int SourceAttractionID, int DestinationAttractionID, List<userTable_OnlyId> AttractionID, string StartDate, string StartTime)
        {
            try
            {
                return _businessAccess.Public_GetOrderOfAttractionVisit(TravelModeId, SourceAttractionID, DestinationAttractionID, AttractionID, StartDate, StartTime);
            }
            catch (Exception ex)
            {
                throw ex;
